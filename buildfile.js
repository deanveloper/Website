...
const ourFilePath = process.argv[2];
if (!ourFilePath) {
    throw new Error("File must be an argument!");
}

fs.readFile(ourFilePath, function (err, fileData) {
    if (err) {
        throw err;
    }

    // If our file is an HTML file, parse the meta file and add onto our html file!
    if (path.extname(ourFilePath) === ".html") {
        let $ = cheerio.load(fileData);
        let newFile = fileData;
        const metaFile = path.join(path.dirname(ourFilePath), "meta.json");
        const scriptBundle = path.join(path.dirname(ourFilePath), "bundle.min.js");
        const head = $("head");
